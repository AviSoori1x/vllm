...
from .pixtral import PATCH_MERGE, PatchMerger, VisionEncoderArgs
from .pixtral import VisionLanguageAdapter, VisionTransformer
from .utils import (flatten_bn, init_vllm_registered_model, maybe_prefix,
                    merge_multimodal_embeddings)
# Import components from Voxtral and Pixtral
from .voxtral import AudioLanguageAdapter, VoxtralEncoderModel
from .voxtral import VoxtralProcessorAdapter
...
class OmnistralProcessorAdapter:
    ...
    def _audio_processor(self) -> AudioEncoder:
        ...
        if not isinstance(audio_encoder, AudioEncoder):
            required_attrs = [
                "special_ids", "audio_config", "next_multiple_of_chunk_frames",
                "pad"
            ]
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if all_tokens:
            input_ids = torch.tensor(all_tokens)[None].expand(
                len(text) if text else 1, -1)
        else:
...
class OmnistralDummyInputsBuilder(
        BaseDummyInputsBuilder[OmnistralProcessingInfo]):
...
class OmnistralMultiModalProcessor(
        BaseMultiModalProcessor[OmnistralProcessingInfo]):
...
class OmnistralForConditionalGeneration(nn.Module, SupportsMultiModal,
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        if not hasattr(config, 'audio_config'):
            raise ValueError(
                "Omnistral requires audio_config in model configuration")
        if not hasattr(config, 'vision_config'):
            raise ValueError(
                "Omnistral requires vision_config in model configuration")
    ...
    def _merge_multimodal_embeddings_safe(
        ...
        if all_embeds.shape[0] == len(positions):
            ...
            logger.warning(
                ...
                "Padding with zeros.")
            padded_embeds = torch.zeros((len(positions),
    ...
    def _process_image_inputs(self, **kwargs) -> list[torch.Tensor]:
        ...
        if (self.vision_args.add_pre_mm_projector_layer_norm
                and hasattr(self, 'pre_mm_projector_norm')):
            image_features = self.pre_mm_projector_norm(image_features)

        # Apply optional patch merging
        if (self.vision_args.mm_projector_id == PATCH_MERGE
                and hasattr(self, 'patch_merger')):
    ...
    def _count_token_groups(self, input_ids: torch.Tensor,
                            token_id: int) -> int:
    ...
    def get_input_embeddings(
        ...
        if has_audio and not has_image:
            ...
            # Correctly count audio inputs using begin_audio token
            ...
            # Split embeddings
            # Audio comes first in get_multimodal_embeddings
            if num_audio_inputs <= len(multimodal_embeddings):
                ...
                logger.info(
                    f"Split embeddings: {len(audio_embeds)} audio, {len(image_embeds)} image"
                )
                ...
                logger.error(
                    f"Found {num_audio_inputs} audio start tokens but only {len(multimodal_embeddings)} embeddings. "
                    "Falling back to size heuristic.")
                # Fallback to heuristic if something is wrong
    ...
    def load_weights(self, weights: Iterable[tuple[str,
        ...
        # Optional components
        patch_merger_dict = dict(self.patch_merger.named_parameters()) if (
            self.vision_args.mm_projector_id == PATCH_MERGE
            and hasattr(self, 'patch_merger')) else dict()
        pre_mm_projector_norm_dict = dict(
            self.pre_mm_projector_norm.named_parameters()) if (
                self.vision_args.add_pre_mm_projector_layer_norm
                and hasattr(self, 'pre_mm_projector_norm')) else dict()
        ...
        def llm_weights_generator():
            ...
            for name, w in weights:
                ...
                is_audio_encoder = (name.startswith("mm_whisper_embeddings")
                ...
                is_vision_encoder = name.startswith(
                    "mm_embeddings.vision_encoder")
                is_vision_lang_adapter = name.startswith(
                    "mm_embeddings.vision_language_projection")
                is_patch_merger = name.startswith("mm_embeddings.patch_merger")
                is_pre_mm_projector_norm = name.startswith(
                    "mm_embeddings.pre_mm_projector_norm")
                ...
                if is_vision_lang_adapter:
                    ...
                    if mapped_name in vision_lang_adapter_dict:
                        ...
                        loaded_weights.add(
                            f"vision_language_adapter.{mapped_name}")
                    continue
                ...
                if is_pre_mm_projector_norm and pre_mm_projector_norm_dict:
                    ...
                    if trimmed_name in pre_mm_projector_norm_dict:
                        ...
                        # Add the actual model parameter name
                        loaded_weights.add(
                            f"pre_mm_projector_norm.{trimmed_name}")
                    continue
